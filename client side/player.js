...
for(var i=0; i<25; i++)
{
    objarr[i].loadTexture("cartexture");

}

roadobj.loadTexture("roadtexture");
skyobj.loadTexture("skytexture");
...
function GpsPlayer(arg) {
    ...
    this.engine.addLight(vec3.fromValues(30, 30, 0), vec3.fromValues(1.0, 0.0, 0.0));
    this.engine.addLight(vec3.fromValues(0, 20, 22), vec3.fromValues(1.0, 0.0, 0.0));


    ///init Ui
    this.playbut.button({
        label: "Play"
    }).click(this.playButFun.bind(this));
    this.renderbut.button({
        label: "Stop Rendering"
    }).click(this.renderButFun.bind(this));
    this.modelbut.button({
        label: "change model"
    }).click(this.modelButFun.bind(this));
    this.resetbut.button({
        label: "reset model"
    }).click(this.resetButFun.bind(this));
    this.rotatebut.button({
        label: "rotate model"
    }).click(this.rotateButFun.bind(this));
...
GpsPlayer.prototype.mainLoop = function () {
    if (!this.mainloop) {
...
GpsPlayer.prototype.drawObjects = function(engine) {
    translation+=0.2;
    if(translation > 5)
        translation = 0;
    ...
    this.engine.pushModelMat();
    mat4.rotate(this.engine.modelmat, this.engine.modelmat, 3.1415/2.0 , vec3.fromValues(1, 0, 0));
    mat4.rotate(this.engine.modelmat, this.engine.modelmat, 3.1415 , vec3.fromValues(0, 1, 0));
    //mat4.rotate(this.engine.modelmat, this.engine.modelmat, 1.1415 , vec3.fromValues(1, 0, 0));
    mat4.scale(this.engine.modelmat, this.engine.modelmat, vec3.fromValues(2,2,2));
    this.engine.drawObject(skyobj); 
    this.engine.popModelMat();
    ...
    //mat4.scale(this.engine.modelmat, this.engine.modelmat, vec3.fromValues(5,5,5));

    this.engine.pushModelMat();
    mat4.rotate(this.engine.modelmat, this.engine.modelmat, this.state.modelangle, vec3.fromValues(0, 1, 0));
	this.engine.drawObject(objarr[0]); //this.objarr[this.state.currentframe]);
    this.engine.popModelMat();

    mat4.translate(this.engine.modelmat,this.engine.modelmat,vec3.fromValues(0,0,translation));
    mat4.scale(this.engine.modelmat, this.engine.modelmat, vec3.fromValues(1,1,5));
    this.engine.drawObject(roadobj);
}
...
GpsPlayer.prototype.updateDisplay = function (obj) {
    ...
    //first of all, select the light source and draw the objects
    for(var i=0; i< this.engine.lightsCount(); i++)
    {
    	//console.log(i.toString());
    	this.engine.selectLightSource(i);
    	this.drawObjects(); //to calculate the z buffer in that source
    	//console.log("done for light ",i.toString());
    }

    this.engine.selectScreen();

    mat4.GPSlookAt(this.engine.viewmat, vec3.fromValues(0.2, 3, -10), vec3.fromValues(0, 0, 0), vec3.fromValues(0, 1, 0));
